...
public class TaskScheduler{
    ...
    static void scheduler(String file1, String file2, int m){
        ...
        while(file1_scanner.hasNext())   //firstly, add all the tasks into a priority task queue. The key of this queue is the release time.
        {
            ...
            /*
             * insert a task into the task queue and use the release time as the key.Since we have n tasks,
             * and each insert cost O(logn) time due to the up-heap opeartions. So for n tasks, the overall
             * time cost is O(nlogn).
             */
            task_queue.insert(release_time.intValue(), tmp);
        }
        
        /*
         *secondly, we schedule the tasks with time sequence. The task_tmp is used to temporarily store
         *the tasks removed from task_queue. The initial value of it is the first task.
         */
        ...
        StringBuffer write_buffer_string = new StringBuffer("");
        ...
        while((!task_queue.isEmpty())||(!schedule_queue.isEmpty())){
            ...
            {
                /*
                 * since all the tasks from task_queue would be removed and all of them would finally be added
                 * into the schedule_queue. So the removes and the insertions cost O(nlogn) time, respectively.
                 */
            ...
            for(int j = 0 ; j < m; j++)
            {
                if(!schedule_queue.isEmpty())
                {
                    ...
                    if(tmp_sche_result.getKey() <= cur_time)
                    {
                        ...
                        write_buffer_string.delete(0,write_buffer_string.length());
                        write_buffer_string.append("No feasible schedule exists");
                        try{
                            out_file.write(write_buffer_string.toString());
                         ...
                         * the write buffer which would be written into output file later. Since if the inputs are 
                         * all valid, then all the tasks would be finally removed from the schedule_queue. So the time 
                         * cost for this removes is O(nlogn).
                         */
                        
                        System.out.printf("task for time %d is %s\n", cur_time, tmp_sche_result.getValue().task_name);
                        write_buffer_string.append(tmp_sche_result.getValue().task_name).append(" ").append(cur_time).append(" ");
                    }
                }
            }
            cur_time++;
        ...
        System.out.println(write_buffer_string.toString());
    }
    
    public static void main(String[] args)
    {
        
        TaskScheduler.scheduler("/Users/wuhuijun/Documents/workspace/java/comp9024_assignment3/src/file1.txt", "/Users/wuhuijun/Documents/workspace/java/comp9024_assignment3/src/file2.txt", 2);
